    ...
    <table-pannel ref="tablePannel" @on-ok="onOk" @on-cancel="onCancel">
        ...
        <slot slot="action">
            ...
            <table>
                <tr>
                    <td>
                        <Button type="primary" @click="onAddMenu" v-auth="['sysMenu:add']">添加菜单</Button>
                    </td>
                    <td>
                        <Poptip transfer confirm title="确定删除吗？" @on-ok="onDeleteBatchMenu">
                            <Button v-auth="['sysMenu:delete']" type="error" ghost
        ...
        <slot slot="grid">
            <vxe-table ref="modelTable" border stripe resizable highlight-current-row show-header-overflow show-overflow
                ...
                <vxe-table-column field="action" title="操作" align="center">
                    <template v-slot="{ row }">
                        <a href="javascript:void(0)" @click="onEditMenu(row)" v-auth="['sysMenu:edit']">
                            编辑
                        </a>
                       <div class="split"></div>
                        <a href="javascript:void(0)" @click="onAddChildMenu(row)" v-auth="['sysMenu:addChild']">
                            添加子菜单
                        </a>
                         <div class="split"></div>
                        <Dropdown transfer v-auth="['sysMenu:delete','sysMenu:frozen']">
