<template>
    <table-pannel ref="tablePannel" @on-ok="onOk" @on-cancel="onCancel" :show-ok-button="showOkButton">
        <slot slot="alert">
            <Alert show-icon>字典管理：用于系统标准字典的统一维护，支持添加字典类型、字典配置、删除字典类型等功能</Alert>
        </slot>
        <slot slot="filter">
            <table>
                <tr>
                    <td>
                        <Select clearable @on-change="onSearch" placeholder="请选择字典类型分组"
                            v-model="modelTable.filter.dictTypeGroup" style="width:166px">
        ...
        <slot slot="action">
            ...
            <table>
                <tr>
                    <td>
                        <Button type="primary" @click="onAddDictType" v-auth="['sysDictType:add']">添加字典类型</Button>
                    </td>
                    <td>
                        <Poptip transfer confirm title="确定删除吗？" @on-ok="onDeleteBatchDictType">
                            <Button type="error" v-auth="['sysDictType:delete']" ghost
        ...
        <slot slot="grid">
            <vxe-table ref="modelTable" border stripe resizable highlight-current-row show-header-overflow show-overflow
                ...
                <vxe-table-column field="action" width="240" title="操作" align="center">
                    <template v-slot="{ row }">
                        <a href="javascript:void(0)" @click="onEditDictType(row)" v-auth="['sysDictType:edit']">
                            编辑
                        </a>
                        <div class="split"></div>
                        <a href="javascript:void(0)" @click="onEditDict(row)" v-auth="['sysDictType:dict']">
                            字典配置
                        </a>
                        <div class="split"></div>
                        <Dropdown transfer v-auth="['sysDictType:frozen','sysDictType:delete']">
